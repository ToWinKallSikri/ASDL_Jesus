...
public class Shelf {
    ...
    public boolean addItem(ShelfItem i) {
        ...
        if(i.getWeight() + getCurrentTotalWeight() > maxTotalWeight ||
                i.getOccupiedSurface() + getCurrentTotalOccupiedSurface() > maxOccupableSurface ||
                i.getLength() > maxLength || i.getWidth() > maxWidth)
            // limiti che non si possono superare
            throw new IllegalArgumentException("Limiti non superabili");
        if(this.numberOfItems == this.items.length){
            ShelfItem [] newArray = new ShelfItem[this.numberOfItems*2];
            for(int j = 0; j < this.numberOfItems; j++)
                newArray[j] = this.items[j];
            this.items = newArray;
            // in base alla quantità di item, se raggiungo la quantità massima, creo un array di dimensione doppia
        }
        this.items[this.numberOfItems++] = i;
        // aggiungo elementi al nuovo array
        return true;
    }
    ...
    public ShelfItem search(ShelfItem i) {
        if (i == null)
            throw new NullPointerException("L'oggetto passato è null");
        for (ShelfItem j : items)
            if (i.equals(j))
                return j;
        return null;
    }
    ...
    public double getCurrentTotalOccupiedSurface() {
        double currOccupiedSurface = 0;
        for ( int i = 0; i < numberOfItems; i++)
            currOccupiedSurface += items[i].getOccupiedSurface();
        return currOccupiedSurface;
    }
